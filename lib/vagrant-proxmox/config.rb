module VagrantPlugins
	module Proxmox
		class Config < Vagrant.plugin('2', :config)
			...
			# The virtual machine type, e.g. :qemu or :lxc
			#
			# @return [Symbol]
			attr_accessor :vm_type

			# The lxc os template to use for the virtual machine
			#
			# @return [String]
			attr_accessor :lxc_os_template

			# The lxc os template file to upload and use for the virtual machine
			#
			# @return [String]
			attr_accessor :lxc_template_file

			# Should the lxc os template be replaced (deleted before upload)?
			#
			# @return [Boolean]
			...
			def validate machine
				...
				if @vm_type == :lxc
					errors << I18n.t('vagrant_proxmox.errors.no_lxc_os_template_or_lxc_template_file_specified_for_type_lxc') unless @lxc_os_template || @lxc_template_file
				end
				if @vm_type == :qemu
